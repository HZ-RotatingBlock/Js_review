          ...
          <div class="checkout-addr">
            ...
            <div class="check-step">
              <ul>
                ...
                <li><span>确定</span>付款</li>
                <li><span>订单</span>结果</li>
              </ul>
            </div>

            <!-- address list -->
            <div class="page-title-normal checkout-title">
              <h2><span>我的收获地址</span></h2>
            </div>
                ...
                <ul>
                  <li v-for="(item,index) in addressListFilter" :class="{'check':checkIndex===index}" @click="checkIndex=index;selectedAddressId=item.addressId">
                    ...
                    <div class="addr-opration addr-set-default">
                      <a href="javascript:;" class="addr-set-default-btn" v-if="!item.isDefault" @click="setDefault(item.addressId)"><i>设为默认收货地址</i></a>
                    </div>
                    <div class="addr-opration" v-if="item.isDefault">当前默认收获地址</div>
                  </li>
                  <li class="addr-new">
            ...
            <div class="shipping-method-wrap">
              <div class="shipping-method">
                <ul>
                  <li class="check">
                    <div class="name">标准物流</div>
                    <div class="price">免费</div>
                    <div class="shipping-tips">
                      <p>确认订单之后，商品会在1-7个工作日之间到达</p>
                    </div>
                  </li>
                </ul>
              </div>
      ...
      init () {
        axios.get('/users/addressList').then((response) => {
          ...
          this.addressList.forEach((item, index) => {
            // 判断哪一个是当前默认设置的收货地址并默认选中，同时以备当用户不进行地址重选而直接点击下一步时传递address参数
            if (item.isDefault) {
